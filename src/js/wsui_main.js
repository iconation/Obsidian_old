...
const ObsidianManager = require('./ws_manager');
const config = require('./ws_config');
const AgGrid = require('ag-grid-community');
const wsmanager = new ObsidianManager();
...
const ObsidianAddressbook = require('./ws_addressbook');
...
let addressBook = new ObsidianAddressbook(ADDRESS_BOOK_DEFAULT_PATH);
...
{
    superagent.get('https://api.github.com/repos/iconation/obsidian/releases/latest').end((err, res) => {
        if (!err && res) {
            let result = JSON.parse (res.text);
            let tag = result.tag_name;
            if (settings.get('version') != tag)
            {
                wsutil.showToast ('A new version of Obsidian is available (' + tag + ') ! '
                            +     'Checkout <a href="#" class="dont-color-link" id="go-latest-release-github">https://github.com/iconation/obsidian</a>', 
                            20 * 1000);
                ...
                goLatestRelease.addEventListener('click', (event) => {
                    shell.openExternal ('https://github.com/iconation/Obsidian/releases/latest');
                });
            }
        }
    });
...
function showInitialPage() {
    ...
    let versionInfo = document.getElementById('obsidianVersion');
    ...
    function loadAddressBook(params) {
        ...
        if (params) {
            ...
            if (params.name === 'default') {
                addressBook = new ObsidianAddressbook(ADDRESS_BOOK_DEFAULT_PATH);
            } else {
                addressBook = new ObsidianAddressbook(params.path, params.name, params.pass);
            }
        }
...
ipcRenderer.on('cleanup', () => {
    ...
    let htmlText = 'Terminating Obsidian...';
